...
// Titles that are still sent by the backend but should NOT also render
// as standalone cards in the generic secondary-stat grid.
...
// Accent colour map for secondary overview stat cards.
...
// Distinct icons for specific secondary stats.
...
// Stats shown as large KPI cards at the top.
...
// Roles allowed to see the full metrics dashboard.
const DASHBOARD_CARD_ROLES = new Set([
  "admin",
  "super_admin",
]);

// Quick actions shown per role.
//
// Keys must match roleName strings from the JWT / roles table exactly.
...
> = {
  reviewer: [
    {
      ...
      description:
        "View the abstracts assigned to you and submit your reviews.",
  ...
  registration_desk_officer: [
    {
      label: "Registration",
      description:
        "Register new attendees for the event.",
...

    {
      label: "Accreditation",
      description:
        "Accredit already-registered participants.",
  ...
  abstract_committee_member: [
...

    {
      label: "Review Abstracts",
      description:
        "Review abstracts assigned to the committee.",
      href: "/icw/abstract-review",
      icon: <CheckCircle2 className="w-6 h-6" />,
    },
  ],
  ...
  author: [
    {
      label: "My Abstracts",
      description:
        "View and manage your abstracts.",
...
function SupervisorActivityBadge({
  status,
}: {
  status: string;
}) {
...
//
// IMPORTANT:
// This now accepts ALL roles and merges the available actions from each
// role. For example:
//
// ["participant", "author"]
//
// will show the My Abstracts action.
function ActionButtonsView({
  ...
  // Merge actions from every assigned role.
  const actions = roles.flatMap(
    (role) => ROLE_ACTIONS[role] || []
  );

  // Remove duplicate links where two roles happen to expose
  // the same destination.
  const uniqueActions = actions.filter(
    (action, index, all) =>
      all.findIndex(
        (item) => item.href === action.href
      ) === index
  );
...
export default function Dashboard() {
  ...
  // Multiple roles from the JWT.
  //
  // Example:
  // ["participant", "author"]
  //
  // instead of the old single:
  // "participant"
  ...
  const normalizePayload = (
    ...
    return {
      ...
      overviewStats:
        Array.isArray(payload?.overviewStats)
          ? payload.overviewStats
          : [],

      supervisorRows:
        Array.isArray(payload?.supervisorRows)
          ? payload.supervisorRows
          : [],

      incidentSnapshot:
        Array.isArray(payload?.incidentSnapshot)
          ? payload.incidentSnapshot
          : [],

      coordinatorNotes:
        Array.isArray(payload?.coordinatorNotes)
          ? payload.coordinatorNotes
          : [],
    };
  };
  ...
  // Only fetch metrics data when at least one of the user's roles
  // is allowed to see the full dashboard.
  useEffect(() => {
    if (!roleReady) {
      return;
    }

    const hasMetricsRole = roles.some(
      (role) =>
        DASHBOARD_CARD_ROLES.has(role)
    );
  ...
  const supervisorRows =
    dashboardData?.supervisorRows ?? [];

  const incidentSnapshot =
    dashboardData?.incidentSnapshot ?? [];

  const coordinatorNotes =
    dashboardData?.coordinatorNotes ?? [];

  const dashboardDate =
    dashboardData?.dashboardDate ?? "--";

  const dayName =
    dashboardData?.dayName ?? "--";

  const programme =
    dashboardData?.programme ?? "--";
  ...
  const present = getStat(
    stats,
    "Total Present for Selected Date"
  );

  const absent = getStat(
    stats,
    "Total Absent for Selected Date"
  );
  ...
  const registered = getStat(
    stats,
    "Registered Participants"
  );

  const accMale = getStat(
    stats,
    "Accredited Male"
  );

  const accFemale = getStat(
    stats,
    "Accredited Female"
  );

  const malePresent = getStat(
    stats,
    "Males Present"
  );

  const femalePresent = getStat(
    stats,
    "Females Present"
  );

  const openIncidents = getStat(
    stats,
    "Open Incidents"
  );

  const abstractsRejected = getStat(
    stats,
    "Abstracts Rejected"
  );
  ...
  const accreditedTotalByGender =
    accMale + accFemale;

  const presentTotalByGender =
  ...
  function goToOverviewDetail(
    item: DashboardOverviewStat
  ) {
    const params =
      new URLSearchParams({
        type: "overview",
    ...
    router.push(
      `/icw/dashboard-detail?${params.toString()}`
    );
  }

  function statOrFallback(
    title: string,
    value: number
  ...
  const hasMetricsDashboard =
    roles.some(
      (role) =>
        DASHBOARD_CARD_ROLES.has(
          role
        )
    );

  // Any non-admin user gets the quick-action dashboard.
  //
  // This is where a participant + author user will now see
  // "My Abstracts".
  ...
  return (
    <Layout>
      ...
      {/* ────────────────────────────────────────────────────────────────
          ...
          This appears when an admin/super_admin ALSO has the author role.
      ...
      */}
      ...
      {roles.includes("author") && (
        <button
          ...
          onClick={() =>
            router.push(
              "/icw/author-dashboard"
            )
          }
          className="w-full mb-6 text-left rounded-2xl bg-white dark:bg-gray-800 border border-violet-100 dark:border-violet-900/40 shadow-sm p-5 transition hover:-translate-y-0.5 hover:shadow-md hover:border-violet-200 dark:hover:border-violet-700/50 focus:outline-none focus:ring-2 focus:ring-violet-500"
        >
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-4 min-w-0">
              <div className="shrink-0 w-12 h-12 rounded-xl bg-violet-50 dark:bg-violet-900/30 flex items-center justify-center text-violet-600 dark:text-violet-300">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  View your submitted abstracts,
                  reviewer feedback, versions and
        ...
        {stats
          .filter(
            (s) =>
              ...
              !HIDDEN_FROM_SECONDARY_GRID.has(
                s.title
              )
          )
          .map((item) => {
            const accent =
              ACCENT_MAP[item.title] ??
              "#6b7280";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
        ...
        <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
            Present vs absent · selected date
          </p>
          ...
          <div className="relative h-48">
            <canvas
              ref={donutRef}
              role="img"
              aria-label={`Present ${present}, Absent ${absent}`}
            >
        ...
        <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
            Accredited vs present by gender
          </p>
          ...
          <div className="relative h-48">
            <canvas
              ref={genderRef}
              role="img"
              aria-label={`Accredited: ${accMale} male, ${accFemale} female. Present: ${malePresent} male, ${femalePresent} female.`}
            >
              Accredited: {accMale} male,{" "}
              {accFemale} female. Present:{" "}
              {malePresent} male,{" "}
              {femalePresent} female.
            </canvas>
          </div>
      ...
      */}
      ...
      <div className="rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm p-5 mb-6">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
          Daily present vs absent across the programme period
        </p>
        ...
        {trend.length === 0 ? (
          ...
          <div className="relative h-44">
            <canvas
              ref={trendRef}
              role="img"
              aria-label="Line chart of daily attendance trend"
            >
              Attendance trend across programme
              period.
            </canvas>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-8">
        ...
        <div className="xl:col-span-2 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm p-5">
          <h2 className="text-sm font-medium text-gray-900 dark:text-white mb-1">
            Sub-CL attendance activity
          </h2>

          <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
            Attendance scans per Sub-CL · selected date
          </p>
          ...
          {supervisorRows.length === 0 ? (
            ...
            <>
              <div
                ...
                style={{
                  height: Math.max(
                    supervisorRows.slice(0, 8).length *
                      44 +
                      24,
                    200
                  ),
              ...
              >
                <canvas
                  ref={supBarRef}
                  role="img"
                  aria-label="Horizontal bar chart of supervisor attendance scan counts"
                >
                  {supervisorRows
                    .slice(0, 8)
                    .map(
                      (s) =>
                        `${s.supervisorName}: ${s.attendanceCount}`
                    )
                    .join(", ")}
                  .
                </canvas>
                ...
                {supervisorRows
                  ...
                  .map((row) => (
                    <div
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-gray-100 dark:border-gray-700 px-2.5 py-1.5"
                    >
                      <span className="text-xs text-gray-700 dark:text-gray-300">
                        {row.supervisorName}
                      </span>

                      <SupervisorActivityBadge
                        status={
                          row.status
                        }
                      />
